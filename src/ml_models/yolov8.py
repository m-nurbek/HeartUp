...
def box_label(image_, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
    lw = max(round(sum(image_.shape) / 2 * 0.003), 2)
    p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
    cv2.rectangle(image_, p1, p2, color, thickness=lw, lineType=cv2.LINE_AA)
    if label:
        tf = max(lw - 1, 1)  # font thickness
        w, h = cv2.getTextSize(label, 0, fontScale=lw / 3, thickness=tf)[0]  # text width, height
        outside = p1[1] - h >= 3
        p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
        cv2.rectangle(image_, p1, p2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(image_,
...
def plot_bboxes(cv2img, boxes, labels=[], colors=[], score=True, conf=None):
    # Define COCO Labels
    if not labels:
    ...
    for box in boxes:
        ...
        if conf:
            if box[-2] > conf:
                ...
                box_label(cv2img, box, label, color)
        else:
            color = colors[int(box[-1])]
            box_label(cv2img, box, label, color)
    ...
    # cv2.imwrite('src/ml_models/result.jpg', cv2img)
    return cv2.imencode('.jpg', cv2img)


model = YOLO("src/ml_models/yolov8n.pt")


async def run_yolo(img: Image, img_path: str = None, score: bool = False, conf: float | None = None) -> tuple:
    if img_path:
    ...
    return plot_bboxes(image, results[0].boxes.data, score=score, conf=conf)


if __name__ == '__main__':
    run_yolo("https://huggingface.co/tasks/assets/image-classification/image-classification-input.jpeg")
    # Result is stored as the result.jpg
